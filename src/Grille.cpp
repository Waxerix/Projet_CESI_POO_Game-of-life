...
void Grille::SetX(int x){x=x;}

void Grille::SetY(int y){y=y;}
...
bool Grille::GetEtat(int x,int y){return grille[y][x]->GetEtat();}
    ...
    for(int y=0;y<x;y++){
        for(int x=0;x;x++){
            if(this->grille[y][x]->GetEtat()){
                ...
                if(this->GetEtat(x,y)){count+=1;}
                else if(NewG[y][x]==nullptr){this->NewEtatMort(x,y,NewG);}

                if(count>3 && count<2){NewG[y][x]=new CellulesMortes;}
...
void Grille::NewEtatMort(int x,int y,std::vector<std::vector<Cellules*>>& NewG){
    if(this->grille[y][x]->GetEtat()){
        int count=0;
        int x1,y1,x2,y2;
        if(y-1<0){y1=this->GetY();}
        else{y1=y-1;}
        if(y+1>this->GetY()){y2=0;}
        else{y2=y+1;}
        if(x-1<0){x1=this->GetX();}
        else{x1=x-1;}
        if(x+1>this->GetX()){x2=0;}
        else{x2=x+1;}
